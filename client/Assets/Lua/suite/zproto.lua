...
function zproto:load(path)
	local proto, err = engine.load(str)
	if not proto then
		return nil, err
	end
...
function zproto:decode(typ, data, sz, offset)
	local record = query(self, typ)
	local ret =  engine.decode(record, data, sz, offset)
	print(typ,json.encode(ret))
	return ret
end
